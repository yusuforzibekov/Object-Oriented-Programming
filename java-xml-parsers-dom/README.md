# Handling Food Items in XML for a Breakfast Menu Using the DOM

The goal of this task is to give you some practice using the DOM to parse XML documents.

Duration: 60 minutes

## Description

Implement a Java program that utilizes a DOM parsing interface to read an XML file containing information about various food items and their properties (e.g., id, name, price, description, and calories). The XML file describes a breakfast menu.
...
- Utilize the DOM interface to process the breakfast menu XML file and create a list of `Food` objects. Use the `BreakfastMenuDomParser` to implement its static method `parseBreakfastMenu` for creating `Food` objects from the parsed XML data.
...
An example of using `BreakfastMenuDomParser`:

```java
String pathToXml = "breakfastmenu.xml";
List<Food> foodList = BreakfastMenuDomParser.parseBreakfastMenu(pathToXml);
